...
class Pad(tk.Frame):
    ...
    def __init__(self, _lexico, parent, *args, **kwargs):
        ...
        self.text3 = tk.Text(self,height=30, width=50, bg="#2B2B2C", fg='#FFFFFF')
        self.text3.focus()
        self.text3.pack(fill="both",expand=True, side = "bottom")
    ...
    def exe_lexico(self, event):
        ...
        for i in range(len(tokens)):
            if tokens[i] != "COMMENT" and tokens[i] != "ERROR" :
                #print (tokens[i])
                data_lex += words[i]
                data_lex += " "

        print (data_lex)
        l_lex,tokens_l = lexico_BNF.main (data_lex)

        pg = sintactico_BNF.Parse_S (l_lex)
        pg.parse()
        parser = pg.get_parser()
        parser.parse(tokens_l)
        

        self.text3.config(state="normal")
        self.text3.delete("1.0","end")
        self.text3.insert("end", "Resultado de la ejecución del Código:\n\n")
        self.text3.insert("end", sintactico_BNF.out_data)
        sintactico_BNF.clear_out()
        self.text3.config(state="disabled")

        #print (sintactico_BNF.t_var)
...
def show_error(self, *args):
    ...
    #print ("Error Sintax")
    sintactico_BNF.t_var = {}
    print (str(args[0]), type(frame))
    
    frame.text3.config(state="normal")
    frame.text3.delete("1.0","end")
    frame.text3.insert("end", "Resultado de la ejecución del Código:\n\n")
    frame.text3.insert("end", str(args[0]))
    sintactico_BNF.clear_out()
    frame.text3.config(state="disabled")
